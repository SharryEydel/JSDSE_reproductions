...
- name: Amelia McNamara ^[amelia.mcnamara@stthomas.edu]
  affiliation: |
    | Computer and Data Sciences Department, University of St Thomas
...
Got warning for a missing font. Line 823 was changed so that I don't have to download a font.
Noticed issue with the references in the paper. Line 229 was changed to codap2021 instead of CODAP2021.
...
here::i_am("paper/Reproduced_PaperDraft.Rmd") # Changed this from "PaperDraft.Rmd" to "paper/Reproduced_PaperDraft.Rmd"
library(kableExtra)
...
allfunctions <- readxl::read_excel(here("data", "processed", "allfunctions_packages.xlsx")) |>
  mutate(section = if_else(section == "tidy", "tidyverse", section)) |>
...
An important pedagogical decision when choosing to teach data analysis is the choice of tool. There has long been a divide between 'tools for learning' and 'tools for doing' data analysis [@mcnamara2015]. Tools for learning include applets, and standalone software like TinkerPlots, Fathom, or their next-generation counterpart CODAP [@konoldmiller2001; @finzer2002a; @codap2021]. Tools for doing are used by professionals, and include software packages like SAS as well as programming languages like Julia, R, and Python.

Many tools for learning were inspired by Rolf Biehler's 1997 paper, "Software for Learning and for Doing Statistics." In it, Biehler called for more attention to the design of tools used for teaching. In particular, he was concerned with on-ramps for students (ensuring the tool was not too complex), as well as off-ramps (using one tool through an entire class, which could also extend further) [@biehler1997]. At the time he wrote the paper it was quite difficult to teach using an authentic tool for doing, because these tools lacked technological or pedagogical on-ramps.
...
In R, the development of less-complex code for students has been under consideration for even longer [@pruimetal2011]. R offers non-standard evaluation, which allows package authors to create new 'syntax' for their packages [@morandatetal2012]. In human language, syntax is the set of rules for how words and sentences should be structured. If someone uses the wrong syntax in human language, people will hear there is something wrong with the speech or writing. However, because human understanding is flexible, the listener will probably still understand the general idea the speaker was trying to convey. Syntax in programming language is more formal-- it governs what code will execute, run, or compile correctly. Using the wrong syntax usually means failing to get a result from the program.

Typically, programming languages have only one valid syntax. For example, an aphorism about Python is "There should be one-- and preferably only one --obvious way to do it" [@peters2004]. But, non-standard evaluation in R has allowed there to be many obvious ways to do the same task [@morandatetal2012, @wickham2019a]. There is some disagreement over whether syntax is a precise term for these differences. Other terms suggested for these variations in valid R code are 'dialects,' 'interfaces,' and 'domain specific languages.' Throughout this paper, we use the term syntax as a shorthand for these concepts. At present, there are three primary syntaxes used: base, formula, and `tidyverse` [@mcnamara2018a].

The base syntax is used by the base R language [@rcoreteam2020], and is characterized by the use of dollar signs and square brackets. The formula syntax uses the tilde to separate response and explanatory variable(s) [@pruimetal2017]. The `tidyverse` syntax uses a data-first approach, and the pipe to move data between steps [@wickhametal2019]. In this paper, the base R pipe `|>` has been used, but in the original pedagogical materials students saw the `magrittr` pipe, `%>%` [@bachewickham2014]. 
...
Examining the results of the study with an eye toward data science pedagogy showed common errors related to conceptions of dataframes and variables. For example, one of the figures from @rafalskietal2019 shows real student code with errors. In the first line of code, the student gets everything correct using formula syntax, with the exception of the name of the dataframe. When that code does not work, they try again using base R syntax, but again get the dataframe name wrong. After both those failures, they appear to fall back on computer science knowledge and try syntax quite different from R. This is consistent with other studies of novice behavior in R [@roberts2015]. It is not clear if this type of error was dependent on the syntax participants were asked to use.
...
This lab course accompanied an introductory statistics course, a first introduction to statistics for all but a handful of students. Typically, students also have no prior experience programming. In the formula section, `r filter(priorexp, Q1=="No", Section.x == "formula") |> pull(n)` had no prior experience programming, and `r filter(priorexp, Q1=="Yes, but not with R", Section.x == "formula") |> pull(n)` had experience programming, but not with R. In the `tidyverse` section, `r filter(priorexp, Q1=="No", Section.x == "tidyverse") |> pull(n)` had no prior experience programming, and `r filter(priorexp, Q1=="Yes, but not with R", Section.x == "tidyverse") |> pull(n)` had experience programming, but not with R. While two additional students in the `tidyverse` section had prior programming experience, the majority of students in both sections had no prior programming experience. 
...
The course these labs accompanied was an introductory statistics course using the Lock5 textbook [@locketal2020]. Weekly lab topics were aligned with lecture content, and standardized across all labs for the semester (13 lab sections using a variety of different software). The topics covered are shown in Table \ref{topics}.
...
4 & Quantitative variables: exploratory data analysis for one quantitative variable. \newline Histograms, dot plots, density plots, and summary statistics like mean, median, and standard deviation. \\  
...
7 & Randomization tests: the use of randomization to perform non-parametric \linebreak hypothesis tests. \\  
8 & Inference for a single proportion: use of the normal distribution to construct 
\linebreak confidence intervals and perform hypothesis tests for a single proportion. \\  
...
11 & \em{No lab, assessment}\\  
12 & \em{No lab, Thanksgiving}\\  
...
```{r prelablength, fig.cap="Length of pre-lab documents, in lines and minutes.", fig.subcap = c("Length of pre-lab RMarkdown documents each week, in lines. Data has been adjusted for the formula section in weeks 8 and 9, because an instructor error led this section to have only one document combining both weeks' work.", "Length of pre-lab videos each week. Outlines delineate multiple videos for a single week."), fig.ncol = 1, fig.align='left',fig.sep = rep("\\newline", 2)}
# 9
## Table 1
...
```{r prepost, fig.cap="Distribution of paired differences for student responses to questions, broken down by section.", fig.asp = 0.8}
# 16
## This is Figure 3
...
```{r post-sentiment}
# 19
# fig.cap = "Responses to the question, ``How was the experience of learning to program in R?''"
x <- prepostsurvey |>
  ...
  summarize(n = n()) |>
  ungroup() |>
  complete(Section, Q23) |>
  mutate(prop = if_else(is.na(n), NA, paste0(round(n / sum(n, na.rm = TRUE), digits = 2)*100, "%")))
  

x |>
  kable(col.names = c("Section", "Response", "n", "Percent"), caption = "Responses to the question, ``How was the experience of learning to program in R?''", booktabs = TRUE, linesep = "", format = "latex") |>
...
# trying a faceted plot for reviewer
youtube_weeks_long <- youtube_weeks |>
  select(week, section, uniques, percent_per_student, mps) |>
  pivot_longer(uniques:mps)

youtube_weeks_long |>
  ggplot() +
  geom_col(aes(x = week, y = value, fill = section), position = "dodge") +
...
# scales are really different, doesn't make sense to do it this way
```


```{r youtube-stats, fig.cap = "Viewing statistics from YouTube. a) shows average number of unique viewers per video. The horizontal line represents the 21 students enrolled in each of the sections, as a baseline for comparison. b) is estimated proportion of YouTube video content watched, per student. This data came from dividing the total amount of time watched by the number of students in each section and the total length of the video(s) for the section that week. c) is estimated number of minutes of YouTube video content watched, per student. This data came from dividing the total amount of time watched by the number of students in each section.",  fig.asp = 1.5}
# 21

# fig.subcap=c("Average number of unique viewers per video. Horizontal line represents the 21 students enrolled in each of the sections, as a baseline for comparison.", "Estimated proportion of YouTube video content watched, per student. This data came from dividing the total amount of time watched by the number of students in each section and the total length of the video(s) for the section that week.", "Estimated number of minutes of YouTube video content watched, per student. This data came from dividing the total amount of time watched by the number of students in each section."),
p1 <- youtube_weeks |>
  ggplot() +
...
p3 <- youtube_weeks |>
  ...
  ylab("Approximate minutes \n of content watched, per student") +
  xlab("Week of semester") +
  theme(
    legend.position = "none",
    legend.title = element_blank()
...
loadfonts()
plot_grid(p1, p2, p3, l1, nrow = 4, rel_heights = c(1,1,1.1,0.2), vjust = c(0.9, 1, 1), labels = c("a)", "b)", "c)", NULL), label_fontface = "plain", label_fontfamily = "Arial")
```
...
```{r rstudio-month, fig.cap="Hours of compute time each section spent on RStudio Cloud, per month of the semester. Students in the \\texttt{tidyverse} section appear to be spending more time on RStudio Cloud, particularly in the months of October and December."}
# 25
## This is figure 5
...
ggplot(rstudio_cloud) +
  ...
  ylab("") +
  xlab("Hours of compute time on RStudio Cloud per student.") +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
    axis.text.y = element_blank(),
...
# 27
rstudio_cloud_month <- rstudio_cloud |>
  group_by(section, month) |>
  summarize(
...
options(knitr.kable.NA = "missing")
rstudio_cloud_assignment <- rstudio_cloud |>
  group_by(section, month) |>
  summarize(mean = mean(amount)) |>
  left_join(topics2) |>
  mutate(approx_per = round(mean / n, digits = 2)) |>
  select(section, month, n, approx_per) |>
  ...
  select(month:tidyverse, difference, n) |>
  pivot_longer(-c(month, n), names_to = "section", values_to = "num") |>
  left_join(rstudio_cloud_month) |>
  mutate(n = as.character(n), num = as.character(num)) |>
  mutate(n = if_else(section=="difference", "", n), info = if_else(section=="difference", "", info))

rstudio_cloud_assignment |>
  select(month, section, info, n, num) |>
  kable(col.names = c("", "", "mean compute time (sd)", "number of assignments", "approximate time per assignment"), caption = "RStudio Cloud usage statistics. Average use per student per month, broken down by section. To find approximate time per assignment, we have divided each month's average by the number of assignments due in the month. The difference between the sections is also computed, and converted into minutes.", booktabs = TRUE) |>
  collapse_rows(columns = 1, valign = "top", latex_hline = "major") |>
  column_spec(3:5, width = c("1.2in", "0.9in", "1.5in"))
...
bothfunctions2 <- formulafunctions |>
  inner_join(tidyfunctions, by = join_by(text, package))
```
...
However, some functions were only shown once over the course of the entire semester. There were `r dim(filter(allfunctions, n == 1, section == "formula"))[1]` functions shown only one time in the formula section, and `r dim(filter(allfunctions, n == 1, section == "tidyverse"))[1]` functions only shown once in the `tidyverse` section.
...
long_caption <- paste0("Top 15 functions in each section. The formula section saw a total of ", filter(num_reused, section == "formula") |> pull(n_fun), " functions and the \\texttt{tidyverse} section saw ", filter(num_reused, section == "tidyverse") |> pull(n_fun), ". Notice that the \\texttt{tidyverse} section has many more functions repeated more than 10 times during the semester. Overall, ",  filter(num_reused, section == "formula") |> pull(prop), " of formula functions were repeated, and ", filter(num_reused, section == "tidyverse") |> pull(prop), " of \\texttt{tidyverse} functions were.")
```

```{r common-functions, fig.cap = long_caption}
# 35
plot_grid(p_formula_functions, p_tidyverse_functions) +
  draw_figure_label(label = "Number of usages", position = "bottom")
```

In Figure \ref{fig:common-functions} we can see the 15 most-used functions in each section. The full list of functions and their usage data is available in the GitHub repo for this paper (\url{https://github.com/AmeliaMN/ComparingSyntaxForModeling}).  Both sections used the `library()` command three times per lab, so those counts are identical between the sections. Similarly, both sections read in data once per lab, so the count for `read_csv()` in the `tidyverse` section is the same as the count for `read.csv()` in the formula section. The difference in rank for the data-reading code (`read.csv()` was one of the third-most-common functions for the formula section, but `read_csv()` was one of the ninth-most-common for the `tidyverse` section) makes it clear there are differences in the pattern of function usage between the sections.

<!-- (REDACTED) -->

In the formula section, the summary statistic `mean()` was the second-ranked function, used `r filter(allfunctions, text == "mean", section == "formula")$n` times. There were four functions used `r filter(allfunctions, text == "tally", section == "formula")$n` times-- `tally()`, `opts_chunk()` (a function in the `knitr` options included in the top of each RMarkdown document) , the aforementioned `read.csv()` and `gf_histogram()`.
...
Interestingly, there was some overlap in the functions students saw in both sections. In fact, there were `r nrow(bothfunctions)` function names both sections saw, including helper functions like `library()`, `set.seed()`, and `opts_chunk()`, modeling-related functions like `aov()`, `lm()`, `summary()` and `predict()`, and statistics like `mean()`, `sd()`, and `cor()`. 

```{r}
# 36
tidyfunctions_unique <- tidyfunctions |>
  anti_join(bothfunctions, by = "text")
tidyfunctions_unique2 <- tidyfunctions |>
  anti_join(bothfunctions, by = join_by(text, package == package.x))
formulafunctions_unique <- formulafunctions |>
  anti_join(bothfunctions, by = "text")
formulafunctions_unique2 <- formulafunctions |>
  anti_join(bothfunctions, by = join_by(text, package == package.y))
```

However, while both sections saw `mean()`, the `tidyverse` section used the \textcolor{grey}{base::}`mean()` function and the formula section saw the `mosaic` masked version of mean, \textcolor{grey}{mosaic::}`mean()`. If we count the masked version of functions separately, the overlap between sections reduces to `r nrow(bothfunctions2)`. If we consider masked versions of functions different, students in the formula section saw `r nrow(formulafunctions_unique2)` functions apart from the set both sections saw, while the `tidyverse` section saw `r nrow(tidyfunctions_unique2)` functions beyond the common set. Again, this makes sense based on the way `tidyverse` code is structured.
...
The comparison also underscores the fact that while instructors may say they are teaching `tidyverse` or formula syntax, they are ultimately teaching R. Both sections saw `r nrow(bothfunctions2)` common functions, most of them from base R (the `base`, `utils` or `stats` packages that come standard with the language). 
...
```{r tally-ex1, ref = "tally-ex1", codecap = "Making tables of one and two categorical variables using the formula syntax and \\texttt{tally()} from the \\texttt{mosaic} package.", echo = TRUE, eval = FALSE}
# 39
...
```{r tidy-tally1, ref = "tidy-tally1", codecap = "Computing summary statistics for one and two categorical variables in the \\texttt{tidyverse} syntax. These functions come from the \\texttt{dplyr} package of the \\texttt{tidyverse}.", echo = TRUE, eval = FALSE}
# 43
penguins |>
  group_by(island) |>
  summarize(n = n())
...
```{r tidy-tally2, ref = "tidy-tally2", codecap = "Computing summary statistics for two categorical variables in the \\texttt{tidyverse} syntax.", echo = TRUE}
# 44
penguins |>
  group_by(island, species) |>
  summarize(n = n()) |>
...
```{r tidy-tally3, ref = "tidy-tally3", codecap = "Computing summary statistics for two categorical variables in the \\texttt{tidyverse} syntax, with variables swapped.", echo = TRUE}
# 45
penguins |>
...
```{r formulamean, ref= "formulamean", codecap = "Using the \\texttt{mosaic} \\texttt{mean()} function to compute the mean. \\texttt{mosaic} masks the base version of \\texttt{mean()}, and makes it formula-aware.",  echo = TRUE, eval = FALSE}
# 47
mean(~bill_length_mm, data = penguins, na.rm = TRUE)
```
...
```{r meantidy, ref = "meantidy", codecap = "Finding the mean using \\texttt{tidyverse} syntax. The \\texttt{drop\\_na()} and \\texttt{summarize()} functions are from the \\texttt{dplyr} package, \\texttt{mean()} comes from base R.", echo = TRUE, eval = FALSE}
# 49
penguins |>
  drop_na(bill_length_mm) |>
  summarize(mean(bill_length_mm))
...
```{r favstats, ref= "favstats", codecap = "The \\texttt{mosaic} \\texttt{favstats()} function provides many common summary statistics for one quantitative variable. The \\texttt{favstats()} function automatically drops missing values.",  echo = TRUE, eval = FALSE}
# 51
...
```{r tidy-summary1, ref = "tidy-summary1", codecap = "Two approaches for doing summary statistics of one quantitative variable in \\texttt{tidyverse} syntax. The first is quite verbose, the second is more compact but introduces a function never seen again. Both approaches use functions from the \\texttt{dplyr} package.", echo = TRUE, eval = FALSE}
# 53
penguins |>
  drop_na(bill_length_mm) |>
  summarize(
...
# 59
bothfunctions2 |>
  mutate(text = paste0("\\texttt{\\textcolor{grey}{", package, "::}", text, "}", " \\newline")) |>
...
\noindent
Used only in formula.
\columnbreak

\noindent
```{r, results = 'asis'}
# 61
tidyfunctions_unique2 |>
  mutate(text = paste0("\\texttt{\\textcolor{grey}{", package, "::}", text, "}", "\\newline")) |>
  mutate(text = gsub("_", "\\_", text, fixed = TRUE)) |>
  add_row(text = "\\noindent \\newline", .before = 1) |>
  pull(text) |>
